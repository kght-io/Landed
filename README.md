...
- **Ops view** — is the machine still running? Queue depth and how long work has waited,
  failed jobs with their errors, inbox-sync freshness, agent runs, and disk growth —
  all from data the app already records, so nothing leaves your machine.
...
- **Your search profile** — level baseline, included/excluded disciplines, locations, and the
  ...
  (stored per install in the DB). The shipped defaults in [backend/src/db/profile.ts](backend/src/db/profile.ts)
  and [backend/src/fitlab/seed.ts](backend/src/fitlab/seed.ts) are generic, fictional placeholders — uploading a
...
Four flat npm workspaces. The frontend and backend are separate packages that run in a single
`next dev` process — the split is about ownership and enforced dependencies, not deployment.

```
frontend/      @landed/frontend — Next.js App Router: pages, components, hooks, and the
               /api route handlers (the only place the backend meets HTTP)
backend/       @landed/backend — server-only: Drizzle schema + queries, the job queue,
               agent orchestration, on-disk paths. Node-only, never bundled to the browser.
...
mcp/           @landed/mcp — stdio MCP server exposing the app to the Claude Code agent
               (zero-dependency; a thin HTTP client over the frontend, it never opens the DB)
...
`frontend → backend → shared`, never the reverse. See **The workspace boundary** in
