...
function applyIncoming(
  ...
  // A synced move into the interview stage earns a brief too — announced, not enqueued from here;
  // the jobs layer subscribes. See ../db/stage-change.ts.
  if (changes.state === "interview") {
    emitStageChange({ postingId: match.id, companyId: match.companyId, from: prevStage, to: "interview" });
  }
...
function resolveCompany(key: string, name: string, actor: string, source: string, approval?: boolean): { co: CompanyRow; isNew: boolean } {
  ...
  return {
    co: { id, name, tier, careersUrl: null, ats: null, fetchMethod: null, fetchRecipe: null, notes: null, desire: null, slug: null, endpoint: null, targetTitles: null, targetLocation: null, leveling: null, ladderMap: null, lastScrapedAt: null, watchlist: false, cooldownUntil: null, createdAt: ts, updatedAt: ts },
    isNew: true,
  };
}
