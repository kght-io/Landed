...
resolves to ([packages/core/src/config.ts](packages/core/src/config.ts)) — is the **single source of truth** that briefs the
...
- **MCP tools** added/removed/renamed in [apps/mcp/jobhunt-server.mjs](apps/mcp/jobhunt-server.mjs) → update the tool lists.
...
The repo is an **npm-workspaces monorepo**: the frontend and the backend are separate packages that
happen to run in one process (`next dev`). Which package a file belongs in is not cosmetic — it is
enforced (see "The workspace boundary" below).

```
apps/web/         Next.js frontend — pages, components, hooks, and the /api route handlers
apps/mcp/         the stdio MCP server (zero-dep; a thin HTTP client over apps/web)
packages/core/    server-only backend — DB, job queue, agent orchestration, on-disk paths
packages/shared/  client-safe code — domain types, pure logic, formatting (browser-safe)
data/  instructions/  scripts/  tests/      (repo-root, shared by every workspace)
```

- **UI** — Next.js 16 (App Router), React 19, Tailwind 4. Routes in [apps/web/app/](apps/web/app/),
  components in [apps/web/components/](apps/web/components/).
- **Data** — Drizzle ORM over `better-sqlite3`. Schema + queries in
  [packages/core/src/db/](packages/core/src/db/).
- **Job queue** — [packages/core/src/jobs/](packages/core/src/jobs/) is the work spine: jobs are
...
- **Agent surface** — [apps/mcp/jobhunt-server.mjs](apps/mcp/jobhunt-server.mjs) exposes MCP tools; CoWork is
...
- **On-disk paths** — anchor them on `REPO_ROOT`
  ([packages/core/src/paths.ts](packages/core/src/paths.ts)), never `process.cwd()`. npm runs a
  workspace script with cwd set to the *package*, so `next dev` reports cwd = `apps/web`; a
...
- **Untyped boundaries** — agent/JSON results arrive as `unknown`. Coerce them through
  [packages/shared/src/coerce.ts](packages/shared/src/coerce.ts) (`num` never returns NaN; `str`
...
| `shared -/-> core` | at **runtime**; type-only imports are exempt (TypeScript erases them) |
...
`packages/core`. Is it a type, pure logic, or formatting the UI also needs? → `packages/shared`.
Is it a page, component, or route handler? → `apps/web`. Route handlers are the seam: they are the
only place that imports `@landed/core` and turns it into HTTP.

Imports use the package name across a boundary (`@landed/core/db`, `@landed/shared/types`) and a
...
run against every workspace. They are pure and fast — pure logic (`packages/shared/src/coerce.ts`,
`leveling.ts`, `linediff.ts`) and the queue (`packages/core/src/jobs/`) are all directly testable
